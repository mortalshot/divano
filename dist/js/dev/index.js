...
function initSliders() {
  if (document.querySelector(".promo-slider__slider")) {
    new Swiper(".promo-slider__slider", {
      modules: [Pagination],
      observer: true,
      observeParents: true,
      slidesPerView: 1,
      ...
      speed: 800,
      pagination: {
        el: ".promo-slider__slider .swiper-pagination",
        clickable: true
      },
      /* breakpoints: {
      	640: {
  ...
  if (document.querySelector(".widget-offers__slider")) {
    new Swiper(".widget-offers__slider", {
      ...
      speed: 800,
      navigation: {
        prevEl: ".widget-offers__slider-wrapper .swiper-button--prev",
        nextEl: ".widget-offers__slider-wrapper .swiper-button--next"
      },
      pagination: {
        el: ".widget-offers__slider .swiper-pagination",
        clickable: true
      },
      breakpoints: {
        768: {
  ...
  if (document.querySelector(".widget-products__slider")) {
    new Swiper(".widget-products__slider", {
      ...
      speed: 800,
      navigation: {
        prevEl: ".widget-products__slider-wrapper .swiper-nav--prev",
        nextEl: ".widget-products__slider-wrapper .swiper-nav--next"
      },
      breakpoints: {
        480: {
          slidesPerView: 2,
  ...
  if (document.querySelector(".widget-feedback__slider")) {
    new Swiper(".widget-feedback__slider", {
      modules: [Navigation],
      observer: true,
      observeParents: true,
      slidesPerView: 1.2,
      ...
      speed: 800,
      navigation: {
        prevEl: ".widget-feedback__slider-wrapper .swiper-nav--prev",
        nextEl: ".widget-feedback__slider-wrapper .swiper-nav--next"
      },
      breakpoints: {
        575: {
          slidesPerView: 2.3,
  ...
  if (document.querySelector(".widget-recommend__slider")) {
    new Swiper(".widget-recommend__slider", {
      modules: [],
      observer: true,
      observeParents: true,
      slidesPerView: 1.2,
  ...
  if (document.querySelector(".widget-ideas__slider")) {
    new Swiper(".widget-ideas__slider", {
      modules: [Navigation],
      observer: true,
      observeParents: true,
      slidesPerView: 1.15,
      ...
      speed: 800,
      navigation: {
        prevEl: ".widget-ideas__slider-wrapper .swiper-button--prev",
        nextEl: ".widget-ideas__slider-wrapper .swiper-button--next"
      },
      breakpoints: {
        480: {
          slidesPerView: 1.6,
  ...
  if (document.querySelector(".widget-gallery__slider")) {
    const gallerySlider = document.querySelector(".widget-gallery__slider");
...
(function initMobileMenuTopOffset() {
  const menuBody = document.querySelector(".menu__body");
  const menuRoot = document.querySelector("[data-fls-menu]");
  if (!menuBody || !menuRoot) return;
  const mq = window.matchMedia("(max-width: 991.98px)");
  const headerSelectors = [".header-top", ".header-main", ".header-bottom"];
  const getHeadersBottom = () => {
    let bottom = 0;
  ...
  const setTop = () => {
    const top = getHeadersBottom();
    menuBody.style.setProperty("--menu-top", `${top}px`);
  };
  const setTopNextFrame = () => {
    requestAnimationFrame(() => {
      setTop();
  ...
  const enable = () => {
    setTopNextFrame();
    window.addEventListener("resize", setTopNextFrame);
    window.addEventListener("scroll", setTopNextFrame, { passive: true });
    window.addEventListener("load", setTopNextFrame);
    document.addEventListener("click", onMenuToggleClick, true);
  };
  const disable = () => {
    window.removeEventListener("resize", setTopNextFrame);
    window.removeEventListener("scroll", setTopNextFrame);
    window.removeEventListener("load", setTopNextFrame);
    document.removeEventListener("click", onMenuToggleClick, true);
    menuBody.style.removeProperty("--menu-top");
  };
  function onMenuToggleClick(e) {
    if (!mq.matches) return;
    const toggle = e.target.closest(".icon-menu");
    if (!toggle) return;
    setTopNextFrame();
  }
...
function menuInit() {
  document.addEventListener("click", function(e) {
    ...
    const burger = e.target.closest(".icon-menu");
    if (!burger) return;
    bodyLockToggle();
...
function isMenuOpen$1() {
  return document.documentElement.hasAttribute("data-fls-menu-open");
}
...
function initTabsBlock(block) {
  ...
  block.addEventListener("pointerover", (e) => {
    ...
    const trigger = e.target.closest("[data-subtarget]");
    if (!trigger || !block.contains(trigger)) return;
    const key = trigger.dataset.subtarget;
    openSubByKey(key);
  });
  block.addEventListener("click", (e) => {
    if (mql.matches) return;
  ...
  block.addEventListener("click", (e) => {
    ...
    e.preventDefault();
    e.stopPropagation();
    const subBody = close.closest(".header-catalog__subbody");
    if (subBody) {
      clearSubAll();
      return;
    }
...
(function initIdeaHotspots() {
  const root = document.querySelector("[data-fls-cardidea]")?.closest(".ideas") || document;
  if (!root) return;
  const cards = root.querySelectorAll(".idea-card");
  if (!cards.length) return;
  const closeAllInCard = (card) => {
    card.querySelectorAll(".hotspot.is-open").forEach((btn) => {
      btn.classList.remove("is-open");
      btn.setAttribute("aria-expanded", "false");
    });
  };
  const closeAll = () => {
    root.querySelectorAll(".hotspot.is-open").forEach((btn) => {
      btn.classList.remove("is-open");
      btn.setAttribute("aria-expanded", "false");
    });
  };
  ...
  document.addEventListener("click", (e) => {
    if (!e.target.closest("[data-fls-ideas] .idea-card")) {
      closeAll();
      return;
    }
    if (!e.target.closest(".hotspot") && e.target.closest("[data-fls-ideas]")) {
      closeAll();
    }
  });
  document.addEventListener("keydown", (e) => {
...
function initHeaderBottomHoverPill() {
  const catalog = document.querySelector(".header-bottom__catalog");
  if (!catalog) return;
  const leftBlock = catalog.closest(".header-bottom__left");
  if (!leftBlock) return;
  const switches = leftBlock.querySelectorAll(".header-bottom__switch");
  if (!switches.length) return;
  const resetToDefault = () => {
    switches.forEach((b) => b.classList.remove("_active"));
    switches[0].classList.add("_active");
    catalog.style.setProperty("--pill-x", "0px");
  };
  const setPillXFromRight = (btn) => {
    if (btn === switches[0]) {
      catalog.style.setProperty("--pill-x", "0px");
      return;
    }
    ...
    const padL = parseFloat(catalogStyles.paddingLeft) || 0;
    const beforeStyles = getComputedStyle(catalog, "::before");
    const pillW = parseFloat(beforeStyles.width) || 0;
    const distToRight = catalogRect.right - btnRect.right;
    const rightInCatalog = catalogRect.width - distToRight;
    const x = rightInCatalog - pillW + padL;
    catalog.style.setProperty("--pill-x", `${Math.max(0, Math.round(x))}px`);
  };
  const activate = (btn) => {
    switches.forEach((b) => b.classList.remove("_active"));
    btn.classList.add("_active");
    setPillXFromRight(btn);
  };
  resetToDefault();
